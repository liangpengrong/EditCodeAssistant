...
namespace Core.CacheLibrary.OperateCache.DataViewOperateCache {
    public class DataViewCache {
        ...
        /// 缓存操作类型
        /// </summary>
        private static Dictionary<string, DataCacheOperationType> cacheOperationType = new Dictionary<string, DataCacheOperationType>();
        /// <summary>
        /// 将缓存追加到缓存工厂中
        /// </summary>
        ...
        public static void removeCacheFactory(string key) {
            ...
            if(cacheIndexs.ContainsKey(key)) { 
                cacheIndexs.Remove(key);
            }
            if(cacheOperationType.ContainsKey(key)) { 
                cacheOperationType.Remove(key);
            }
