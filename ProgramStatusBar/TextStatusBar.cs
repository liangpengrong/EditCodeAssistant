...
namespace ProgramStatusBar
{
    public partial class TextStatusBar : UserControl
    {
        ...
        private void initStatusStripConfig() {
            statusStrip.Name = DefaultNameCof.toolStart;
        ...
        private List<string[]> itemsDateAll()
        {
            ...
            listStrAll.Add(new string[] { StrutsStripDateLib.ItemName.总字符数, "总字符：0", "总字符" });
            listStrAll.Add(new string[] { StrutsStripDateLib.ItemName.选中字符数, "已选中字符：0", "已选中字符" });
            listStrAll.Add(new string[] { StrutsStripDateLib.ItemName.总行数, "总行：1", "总行" });
            listStrAll.Add(new string[] { StrutsStripDateLib.ItemName.行列数, "行：1，列：1", "行：{1},列：{2}" });
            listStrAll.Add(new string[] { StrutsStripDateLib.ItemName.编码, Encoding.UTF8.BodyName.ToUpper(), "字符编码" });
            listStrAll.Add(new string[] { StrutsStripDateLib.ItemName.大小写状态, "小写", "大小写" });
        ...
        private void toolLabelMouseDown(object sender, MouseEventArgs e)
        {
            ...
            ControlsUtilsMet.getAllControlByType<TextBox>(textList,ControlCache.getSingletonCache(DefaultNameCof.tabContent).Controls);
        ...
        /// </summary>
        /// <returns></returns>
        public static StatusStrip getTextRightMenu() { 
            Control con = ControlCache.getSingletonCache(DefaultNameCof.toolStart);
            if(con == null) {
